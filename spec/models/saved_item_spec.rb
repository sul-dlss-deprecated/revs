...
describe SavedItem do
  ...
  it "should save a favorite in a user's favorite gallery" do
    ...
    expect(SolrDocument.find(@druid1).is_favorite?(@user)).to be_falsey
    expect(SolrDocument.find(@druid2).is_favorite?(@user)).to be_falsey
    ...
    expect(SolrDocument.find(@druid1).is_favorite?(@user)).to be_truthy
    expect(SolrDocument.find(@druid2).is_favorite?(@user)).to be_truthy
  end
