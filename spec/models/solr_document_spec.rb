  ...
  describe "collections" do
    ...
    describe "collection siblings" do
      it "should memoize the solr request to get the siblings of a collection member" do
        ...
        solr = double("solr")
        solr.should_receive(:select).with(:params => {:fq=>"is_member_of_ssim:\"collection-1\" AND ((*:* -visibility_isi:[* TO *]) OR visibility_isi:1)", :sort=>"priority_isi desc",:rows => "20", :start=>"0"}).once.and_return(response)
