class Curator::TasksController < ApplicationController
  ...
  before_filter :get_paging_params
   ...
   def flags
     ...
     @order=sort_field(params[:order],(@tab == @tab_list_user ?  "num_flags DESC" : "flags.created_at DESC")) # default sort depends on tab
   ...
   def annotations
     ...
     @order=sort_field(params[:order],(@tab == @tab_list_all ?  "annotations.created_at DESC" : "num_annotations DESC")) # default sort depends on tab
     ...
     annotations=Annotation.all
      ...
      case @tab
        ...
        when @tab_list_all
          annotations=annotations.joins(:item)
          annotations=annotations.where(['items.title like ? OR annotations.druid=?',"%#{@search}%",@search]) unless @search.blank?
   ...
   def favorites
     ...
     @order=sort_field(params[:order],(@tab == @tab_list_item ?  "num_favorites DESC" : "num_galleries DESC")) # default sort depends on tab
